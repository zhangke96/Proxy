...
uint16_t GetUint16(const char *str) {
  ...
  LOG_DEBUG << "GetUint16 network:" << result << " host:" << ntohs(result);
  return ntohs(result);
}
...
uint32_t GetUint32(const char *str) {
  ...
  LOG_DEBUG << "GetUint32 network:" << result << " host:" << ntohl(result);
  return ntohl(result);
}
...
int32_t Getint32(const char *str) {
  int32_t result;
  memcpy(&result, str, sizeof(uint32_t));
  LOG_DEBUG << "Getint32 network:" << result << " host:" << ntohl(result);
  return ntohl(result);
}
...
uint64_t GetUint64(const char *str) {
  ...
  LOG_DEBUG << "GetUint64 network:" << result << " host:" << be64toh(result);
  return be64toh(result);
#else
  LOG_DEBUG << "GetUint64 network:" << result << " host:" << result;
  return result;
#endif
}
...
bool ProxyMessage::ParseFromStr(const char *str, size_t str_length) {
  LOG_DEBUG << "length: " << str_length << " message head size:" << Size();
  if (str_length < Size()) {
    return false;
  }
  int pos = 0;
  ...
  switch (message_type) {
    ...
    case PROTOBUF_MESSAGE: {
      ...
      if (!body_parse_ret) {
        LOG_DEBUG << "Parse body pb request failed";
        delete body;
        body = nullptr;
      } else {
        LOG_DEBUG << "Parse body pb request succ";
      }
      break;
    }
    case PROTOBUF_RESPONSE: {
      ...
      body_parse_ret = body->ParseFromStr(str + pos, str_length - pos);
      if (!body_parse_ret) {
        LOG_DEBUG << "Parse body pb response failed";
        delete body;
        body = nullptr;
      } else {
        LOG_DEBUG << "Parse body pb response succ";
      }
      break;
    }
    case DATA_REQUEST: {
      ...
      body_parse_ret = body->ParseFromStr(str + pos, str_length - pos);
      if (!body_parse_ret) {
        LOG_DEBUG << "Parse body data request failed";
        delete body;
        body = nullptr;
      } else {
        LOG_DEBUG << "Parse body data request succ";
      }
      break;
    }
    case DATA_RESPONSE: {
      ...
      body_parse_ret = body->ParseFromStr(str + pos, str_length - pos);
      if (!body_parse_ret) {
        LOG_DEBUG << "Parse body data response failed";
        delete body;
        body = nullptr;
      } else {
        LOG_DEBUG << "Parse body data response succ";
      }
      break;
    }
    default: {
      LOG_DEBUG << "unhandle message type:" << message_type;
      break;
    }
  }
...
std::string ProxyMessage::ToString() const {
  ...
  LOG_DEBUG << "ProxyMessage total size:" << Size()
            << " message_type:" << message_type << " body length:" << length
            << " request_id:" << request_id;
  if (body) {
    LOG_DEBUG << "actual body length:" << body->Size();
...
bool PbRequestBody::ParseFromStr(const char *str, size_t str_length) {
  LOG_DEBUG << "length: " << str_length << " minimal size:" << Size();
  if (str_length < Size()) {
    return false;
  }
  int pos = 0;
  length = GetUint32(str + pos);
  pos += sizeof(length);
  LOG_DEBUG << "length: " << str_length << " actual need size:" << Size();
  if (str_length < Size()) {
    return false;
  }
...
std::string PbRequestBody::ToString() const {
  ...
  result.append(reinterpret_cast<char *>(&length_n), sizeof(length_n));
  result.append(data);
  LOG_DEBUG << "PbRequest[Response]Body total size:" << Size()
...
std::string DataRequestBody::ToString() const {
  ...
  result.append(data);
  LOG_DEBUG << "DataRequestBody total size:" << Size() << " length:" << length
...
std::string DataResponseBody::ToString() const {
  ...
  result.append(data);
  LOG_DEBUG << "DataResponseBody total size:" << Size() << " length:" << length
